...
(function() {
  ...
  function setSize(tier, force = false) {
    ...
    // Get current widget position and viewport dimensions
    const container = document.getElementById(WIDGET_ID);
    const rect = container ? container.getBoundingClientRect() : { left: 0, top: 0 };
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
    
    console.log('🔍 setSize Debug - Initial Position:', {
      tier,
      rect,
      viewportWidth,
      viewportHeight,
    ...
    if (tier === SIZE.M) {
      // Constrain width and height to available viewport space
      const maxWidth = Math.min(420, availableWidth);
      const maxHeight = Math.min(560, availableHeight);
      
      const finalWidth = Math.max(320, maxWidth);
      const finalHeight = Math.max(200, maxHeight);
      
      console.log('🔍 Viewport Debug - M Size:', {
        viewportWidth,
        viewportHeight,
      ...
      // Adjust widget container position if it would overflow
      const container = document.getElementById(WIDGET_ID);
      if (container) {
        const newRightEdge = rect.left + finalWidth;
        const newBottomEdge = rect.top + finalHeight;
        ...
        if (newRightEdge > viewportWidth - 20) {
          newLeft = viewportWidth - finalWidth - 20;
          // Ensure we don't go negative
          newLeft = Math.max(20, newLeft);
        }
        
        // Check bottom boundary
        if (newBottomEdge > viewportHeight - 20) {
          newTop = viewportHeight - finalHeight - 20;
          // Ensure we don't go negative
        ...
        if (newLeft !== rect.left || newTop !== rect.top) {
          ...
          console.log('🔧 Adjusted widget position to:', { left: newLeft, top: newTop });
        } else {
          console.log('🔧 Widget position is already correct:', { left: newLeft, top: newTop });
        }
      }
      ...
      // Ensure widget stays within viewport after size change
      const widgetContainer = document.getElementById(WIDGET_ID);
      if (widgetContainer) {
        ensureWidgetInViewport(widgetContainer);
      }
    }
    ...
    if (tier === SIZE.L) {
      ...
      const finalWidth = Math.max(420, maxWidth);
      const finalHeight = Math.max(300, maxHeight);
  ...
  function createWidget() {
    ...
    // Add window resize listener to keep widget in viewport
    ...
    // Add periodic check to ensure widget stays in bounds
    setInterval(() => {
      const widgetContainer = document.getElementById(WIDGET_ID);
  ...
  function ensureWidgetInViewport(container) {
    ...
    // Check if widget is outside viewport bounds or has invalid coordinates
    if (rect.left < 0 || rect.left > viewportWidth || isNaN(rect.left)) {
      newLeft = Math.max(20, viewportWidth - rect.width - 20);
      needsReposition = true;
    }
    
    if (rect.top < 0 || rect.top > viewportHeight || isNaN(rect.top)) {
      newTop = Math.max(20, viewportHeight - rect.height - 20);
      needsReposition = true;
    }
    ...
    if (needsReposition || isNaN(rect.left) || isNaN(rect.top)) {
      container.style.left = newLeft + 'px';
      container.style.top = newTop + 'px';
      container.style.right = 'auto';
      container.style.bottom = 'auto';
      container.style.transform = 'none';
      console.log('🔧 Widget repositioned to stay in viewport:', { left: newLeft, top: newTop });
    }
  }
  ...
  function setupDragAndResize(container) {
    ...
    document.addEventListener('mousemove', (e) => {
      if (isDragging) {
        ...
        // Keep widget within viewport with strict boundaries
        const maxLeft = window.innerWidth - container.offsetWidth;
        const maxTop = window.innerHeight - container.offsetHeight;
        
        // Strict boundary handling - never allow widget to go outside viewport
    ...
    document.addEventListener('mouseup', () => {
      if (isDragging) {
        ...
        // Finalize position with strict boundary enforcement
        const rect = container.getBoundingClientRect();
        const maxLeft = window.innerWidth - container.offsetWidth;
        const maxTop = window.innerHeight - container.offsetHeight;
        
        // Enforce strict boundaries - never allow widget outside viewport
  ...
  function showResults(results, query) {
    ...
    // Expand widget to show results using the proper setSize function
    if (iframe) {
      console.log('🔍 Expanding widget to show results');
      setSize(SIZE.M);
    }
  }
